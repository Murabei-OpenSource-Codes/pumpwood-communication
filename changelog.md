...
- **`disable_etl_trigger`**: query parameter on ``save``, ``delete``,
  and ``execute_action`` (default ``False``).
- **Parallel endpoints**: ``parallel_save``, ``parallel_delete``, and
...
- **`PumpWoodMicroServiceBase.clone`**: create an independent copy of a
  microservice instance with the same connection settings; optionally
  copy the logged-in session for safe use in background threads.
- **ETL types**: ``ETLDimensions``, ``ETLDataInputObject``, ``ETLFact``,
  and ``ETLAuxResults`` dataclasses exported from
